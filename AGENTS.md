# 文件树（file-tree）——LinkDesk 插件仓

> **本文件是给在这个仓里干活的 AI 看的**（Claude Code / Codex / Cursor / …）。人看 `README.md`。
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`file-tree`）。当前版本 `1.0.15`。

## 1. 这是什么

文件树 / 资源管理器。图标栏进侧栏，含「资源管理器」与「搜索」两个视图。

**用户在哪看到它**：侧栏（`viewsContainers.explorer.location: "sidebar"`）；图标栏顶部有入口按钮。
